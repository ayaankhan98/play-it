...
function Board(props) {
  ...
  const handleKeyDown = (event) => {
    ...
    const {x, y, direction} = snake.snakeHead;
    console.log(`Head direction = ${direction}`)
    console.log(`Key direction = ${mapKeyToDirection(event.key)}`)
  ...
  return <div className={styles.board} tabIndex="1" onKeyDown={handleKeyDown} >{test}</div>
} 
